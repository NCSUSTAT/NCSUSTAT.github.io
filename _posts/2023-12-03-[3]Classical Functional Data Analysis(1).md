...
function space spanned by the first M eigenfunctions, and j-th Functional Pricipal component score is expressed as $ z_{j}=\int (X(t)-\mu(t))^{T}H_j(t)dt=\langle\X(t)-\mu(t),H_j(t),\rangle$, where $C(t,t')=\Sigma_j \lambda_j H_j(t) H_j(t)^Tdt$ by Karhunen–Loeve Theorem.
