...
## Estimation of Covariance Function 
 - The covariance function can be decomposed into two parts, a variance function and a correlation structure, i.e., $C(s, t) = \sigma_X (s) \sigma_X(t) \rho(s, t)$, where $\sigma^2_X(t)$ is the variance function of $X$, or more precisely, $\sigma^2_X(t) = E[(X(t) - \mu(t))^2]$, and $\rho(s, t)$ is the correlation function.
   
 - Estimation of variance Function:

   $(\hat{b_0},\hat{b_1}) = \arg \min_{(b_0, b_1) \in R^2} \Sigma_{i=1}^{n} w_i\Sigma_{j=1}^{m_i} Kh_{\sigma}(T_{ij} - t) \left(Y_{ij} - \hat{\mu}(T_{ij})\right)^2 - b_0 - b_1(T_{ij} - t))^2$
