...
- In every figure, the (2,2) element represents the $C(s, t) = \sigma_{X(s)}\sigma_{X(t)}\rho(s, t)$, which generates the data, where the variance function $\sigma^2_X(t) = 1$ , and Matérn correlation function $\rho_{\theta= (1, 1)}$:
